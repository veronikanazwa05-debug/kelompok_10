...
def admin_view_products():
    ...
    print(f"{'ID':<5} {'Nama Produk':<25} {'Stok':<8} {'Harga':<12} {'Kategori':<15} {'Pemilik':<15}")
    print("-" * 90)
    for p in products:
        print(f"{p['id_produk']:<5} {p['nama_produk']:<25} {p['stok']:<8} {p['harga']:<12} {p['nama_kategori']:<15} {p['pemilik']:<15}")
...
def pengelola_lihat_produk():
    ...
    print("-" * 80)
    for p in products:
        print(f"{p['id_produk']:<5} {p['nama_produk']:<25} {p['stok']:<8} {p['harga']:<12} {p['nama_kategori']:<15} {p['diskon']:<10}")
...
def pengelola_edit_produk():
    ...
    diskon_input = input(f"Diskon ({product['diskon']*100}%): ").strip()
    diskon = float(diskon_input)/100 if diskon_input else product['diskon']
...
def kasir_lihat_produk():
    """Lihat daftar produk untuk transaksi"""
    clear_screen()
    ...
    print(f"{'ID':<5} {'Nama Produk':<30} {'Stok':<8} {'Harga':<12} {'Diskon':<10}")
    print("-" * 70)
    for p in products:
        harga_diskon = p['harga'] * (1 - p['diskon'])
        print(f"{p['id_produk']:<5} {p['nama_produk']:<30} {p['stok']:<8} {harga_diskon:>10,.0f} {p['diskon']*100:>8.0f}%")
...
def kasir_tambah_transaksi():
    ...
    while True:
        ...
        print(f"Harga: Rp {produk['harga']:,.0f} (Diskon {produk['diskon']*100:.0f}%)")
        ...
        harga_final = produk['harga'] * (1 - produk['diskon'])
